...
export function TournamentComments({ tournamentId }: TournamentCommentsProps) {
  const { user, isAuthenticated } = useAuth();
  const { toast } = useToast();
  const [newComment, setNewComment] = useState("");

  const { data: comments = [], isLoading } = useQuery<TournamentComment[]>({
    queryKey: ["/api/tournaments", tournamentId, "comments"],
  ...
  const commentMutation = useMutation({
    mutationFn: async (content: string) => {
      return await apiRequest("POST", `/api/tournaments/${tournamentId}/comments`, { content });
    },
    onSuccess: () => {
      setNewComment("");
      queryClient.invalidateQueries({ queryKey: ["/api/tournaments", tournamentId, "comments"] });
      toast({
  ...
  const deleteCommentMutation = useMutation({
    mutationFn: async (commentId: string) => {
      return await apiRequest("DELETE", `/api/tournaments/${tournamentId}/comments/${commentId}`, {});
    },
    onSuccess: () => {
          ...
          {comments.map((comment) => (
            <div key={comment.id} className="flex gap-3 bg-muted/50 rounded-lg p-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <Reactions commentType="tournament" commentId={comment.id} />
              </div>
            </div>
          ))}
        </div>
